"""Pure-Python AES block cipher (encryption only).

GCM needs only the forward cipher, so decryption is intentionally absent.
Implementation favors readability over speed; protected code blobs are
small (kilobytes), where the difference is milliseconds.

Only AES-128 and AES-256 are exposed - the key sizes this project uses.
"""

from __future__ import annotations

_SBOX = [
    ,
]

_RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C, 0xD8]


def _xtime(a: int) -> int:
    a <<= 1
    if a & 0x100:
        a = (a ^ 0x1B) & 0xFF
    return a


def _key_expansion(key: bytes) -> list[list[int]]:
    nk = len(key) // 4
    if nk not in (4, 8):
        raise ValueError("AES key must be 16 or 32 bytes")
    words = [list(key[4 * i : 4 * i + 4]) for i in range(nk)]
    total = 4 * (nk + 7)

    for i in range(nk, total):
        temp = list(words[i - 1])
        if i % nk == 0:
            temp = temp[1:] + temp[:1]
            temp = [_SBOX[b] for b in temp]
            temp[0] ^= _RCON[i // nk - 1]
        elif nk > 6 and i % nk == 4:
            temp = [_SBOX[b] for b in temp]
        words.append([a ^ b for a, b in zip(words[i - nk], temp)])

    return [sum(words[4 * r : 4 * r + 4], []) for r in range(len(words) // 4)]


def _add_round_key(state: list[int], rk: list[int]) -> None:
    for i in range(16):
        state[i] ^= rk[i]


def _sub_bytes(state: list[int]) -> None:
    for i in range(16):
        state[i] = _SBOX[state[i]]


def _shift_rows(state: list[int]) -> None:
    # column-major state; row r shifted left by r
    for r in range(1, 4):
        row = [state[r + 4 * c] for c in range(4)]
        for c in range(4):
            state[r + 4 * c] = row[(c + r) % 4]


def _mix_columns(state: list[int]) -> None:
    for c in range(4):
        col = state[4 * c : 4 * c + 4]
        state[4 * c + 0] = _xtime(col[0]) ^ (_xtime(col[1]) ^ col[1]) ^ col[2] ^ col[3]
        state[4 * c + 1] = col[0] ^ _xtime(col[1]) ^ (_xtime(col[2]) ^ col[2]) ^ col[3]
        state[4 * c + 2] = col[0] ^ col[1] ^ _xtime(col[2]) ^ (_xtime(col[3]) ^ col[3])
        state[4 * c + 3] = (_xtime(col[0]) ^ col[0]) ^ col[1] ^ col[2] ^ _xtime(col[3])


class Aes:
    """Forward-only AES block cipher."""

    def __init__(self, key: bytes):
        self._round_keys = _key_expansion(key)
        self._rounds = len(self._round_keys) - 1

    def encrypt_block(self, block: bytes) -> bytes:
        if len(block) != 16:
            raise ValueError("AES operates on exactly 16-byte blocks")
        state = list(block)
        _add_round_key(state, self._round_keys[0])
        for rnd in range(1, self._rounds):
            _sub_bytes(state)
            _shift_rows(state)
            _mix_columns(state)
            _add_round_key(state, self._round_keys[rnd])
        _sub_bytes(state)
        _shift_rows(state)
        _add_round_key(state, self._round_keys[self._rounds])
        return bytes(state)


def ecb_encrypt(key: bytes, data: bytes) -> bytes:
    """ECB is unsafe for general use; exposed for GCM's internal CTR only."""
    aes = Aes(key)
    out = bytearray()
    for off in range(0, len(data), 16):
        out += aes.encrypt_block(data[off : off + 16])
    return bytes(out)
